...
public class Launch {
	...
	public void afficherMenu() {
		...
		System.out
				.println("4 - Cliquer sur la carte pour obtenir un numero de sommet.");
		System.out
				.println("5 - Charger un fichier de chemin (.path) et le verifier.");
	...
	public static void test_parcours(Graphe G) {
		Node[] tab_nodes = G.getTabNode();
		ArrayList<Arc> liste_arcs;
		float moyenne_nb_succ = 0 ;
		for (int i = 0; i < tab_nodes.length; i++){
			liste_arcs = tab_nodes[i].getArc();
			if (liste_arcs.isEmpty())
				System.out.println("LISTE VIDE SUR SOMMET " + i);
			else {
				System.out.println("Nb successeurs de " + i + " = " + liste_arcs.size());
				moyenne_nb_succ += liste_arcs.size() ; 
			}
		}
		System.out.println("Moyenne du nombre de successeurs : " + moyenne_nb_succ/tab_nodes.length) ;
	...
	public void go() {
		...
		try {
			System.out.println("**");
			System.out
					.println("** Programme de test des algorithmes de graphe.");
			System.out.println("**");
			System.out.println();

			// On obtient ici le nom de la carte a utiliser.
			String nomcarte = this.readarg
					.lireString("Nom du fichier .map a utiliser ? ");
			DataInputStream mapdata = Openfile.open(nomcarte);

			boolean display = (1 == this.readarg
					.lireInt("Voulez-vous une sortie graphique (0 = non, 1 = oui) ? "));
			Dessin dessin = (display) ? new DessinVisible(800, 600)
					: new DessinInvisible();
			...
			while (continuer) {
				...
				case 1:
					algo = new Connexite(graphe, this.fichierSortie(),
							this.readarg);
					break;
				...
				case 5:
					String nom_chemin = this.readarg
							.lireString("Nom du fichier .path contenant le chemin ? ");
